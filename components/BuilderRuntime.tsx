...
export function LeadSignupBlock({ variantStrategy = 'random', headingShort = 'Get SMS Photo Updates', headingLong = 'Get Exclusive Studio37 SMS Updates (Mini Sessions, Promos, Tips)', subheadingShort = 'Be first to know about mini sessions & promos.', subheadingLong = 'Join our SMS list for first-access to mini sessions, seasonal promotions, and professional photography tips. Unsubscribe anytime.', showIframe = 'false', animation = 'fade-in', buttonText = 'Join SMS List', buttonVariant = 'primary' }: {
  ...
  const animClass = animation === 'fade-in' ? 'animate-fadeIn' : animation === 'slide-up' ? 'animate-slideUp' : animation === 'zoom' ? 'animate-zoom' : ''
  let assigned: 'short' | 'long' = 'short'
  if (isClient) {
    const existing = window.localStorage.getItem('lead_signup_variant') as 'short' | 'long' | null
    if (existing) {
      assigned = existing
    } else {
      assigned = variantStrategy === 'random' ? (Math.random() < 0.5 ? 'short' : 'long') : (variantStrategy === 'long' ? 'long' : 'short')
      window.localStorage.setItem('lead_signup_variant', assigned)
    }
  }
  ...
  const revealIframe = () => setIframeVisible(true)
  React.useEffect(() => {
    if (!isClient) return
    // Impression event
  ...
  React.useEffect(() => {
    if (iframeVisible && isClient && (window as any).gtag) {
      ;(window as any).gtag('event', 'lead_signup_block_iframe_visible', { variant: assigned })
    }
  }, [iframeVisible, assigned, isClient])
    ...
    <section className={`py-12 md:py-16 px-6 md:px-8 bg-white ${animClass} rounded-lg border border-amber-200`}> 
      ...
      <div className="max-w-3xl mx-auto">
        ...
        {!iframeVisible && !showFrameProp && (
          <button
            type="button"
            onClick={() => { revealIframe(); if ((window as any).gtag) { (window as any).gtag('event', 'lead_signup_block_cta_click', { variant: assigned }) } }}
